...
class SalesTableWidget(QTableWidget):
    ...
    def setup_table(self):
        self.setColumnCount(5) # Añadido MetodoPago
        self.setHorizontalHeaderLabels(['ID Venta', 'Fecha', 'Vendedor', 'Método Pago', 'Total'])
    ...
    def load_report_data(self, start_date, end_date):
        ...
        if error:
            msg = QMessageBox(self.parent_widget); msg.setIcon(QMessageBox.Icon.Critical)
            msg.setWindowTitle("Error"); msg.setText(f"No se pudo generar el reporte: {error}"); msg.exec()
            self.setRowCount(0); return
        ...
        for row, record in enumerate(data):
            ...
            self.setItem(row, 3, QTableWidgetItem(record.MetodoPago or 'N/A')) # Nueva columna
...
class ReportsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        title = QLabel("Reportes"); title.setObjectName("pageTitle")
        ...
        self.from_date = QDateEdit(); self.from_date.setDate(QDate.currentDate().addMonths(-1)); self.from_date.setCalendarPopup(True)
        self.from_date.setStyleSheet("QDateEdit { background: rgba(51, 65, 85, 0.5); border: 2px solid rgba(100, 116, 139, 0.3); border-radius: 12px; padding: 14px 16px; font-size: 15px; color: #F1F5F9; } QDateEdit:focus { border: 2px solid #6366F1; } QDateEdit:hover { border: 2px solid rgba(99, 102, 241, 0.5); } QDateEdit::drop-down { border: none; width: 30px; } QDateEdit::down-arrow { image: none; border-left: 5px solid transparent; border-right: 5px solid transparent; border-top: 6px solid #94A3B8; margin-right: 8px; }")
        controls_layout.addWidget(self.from_date)
        controls_layout.addWidget(QLabel("Hasta:"))
        self.to_date = QDateEdit(); self.to_date.setDate(QDate.currentDate()); self.to_date.setCalendarPopup(True)
        ...
        self.sales_table = SalesTableWidget(self); layout.addWidget(self.sales_table)
        self.refresh_data() 
